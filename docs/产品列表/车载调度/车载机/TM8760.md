...
![设置车辆信息](/产品列表/车载调度/车载机/TM8760/设置车辆编号.png )
...
| 视频设置 | 说明 |
| :-: | - |
| ![输出参数](/产品列表/车载调度/车载机/TM8760/视频设置-输出参数.png =300x) | **输出参数**：<br /><br />设置的是显示屏上面，视频通道的画面亮度、色调、对比度、饱和度 |
| ![录像通道](/产品列表/车载调度/车载机/TM8760/视频设置-录像通道.png =300x) | **录像通道**：<br /><br />设置需要哪个通道录像，车载机安装有几路摄像头，就勾选几个通道<br /><br />备注：如果没有安装摄像头的通道也勾选，那么该路摄像头也会录像，占用硬盘空间 |
| ![镜像](/产品列表/车载调度/车载机/TM8760/视频设置-镜像.png =300x) | **镜像**：<br /><br />设置某个通道左右翻转 |
| ![翻转](/产品列表/车载调度/车载机/TM8760/视频设置-翻转.png =300x) | **翻转**：<br /><br />设置某个通道上下翻转 |
| ![其他](/产品列表/车载调度/车载机/TM8760/视频设置-其他.png =300x) | **其他**：<br /><br />1、视频默认通道：<br /> - 手动按‘视频’按键，进入视频界面后默认显示的通道<br /><br />2、拍照默认通道：<br /> - 在视频界面点击‘抓拍’按键，抓拍该通道的图片 |

### 3、通讯设置

|通讯设置|说明|
|:-:|-|
|![拨号设置](/产品列表/车载调度/车载机/TM8760/通讯设置.png =300x)|**拨号设置**：<br /><br />1、网络模式：全网通，默认‘自动’>。<br /><br />2、接入点：默认‘3gnet’，如果客户使用专网卡，需要设置此处为特殊的接入点，运营商会提供。<br /><br />3、输入音量，输出音量：调整通讯模块的音量，一般不需要调整。<br /><br />4、通话时长：调整GSM通话时间，默认不需要调整。|
|![通讯设置-无线设置](/产品列表/车载调度/车载机/TM8760/通讯设置-无线设置.png =300x)|**无线设置**：<br /><br />- 车载机无线WIFI信息|
|![通讯设置-定位模块](/产品列表/车载调度/车载机/TM8760/通讯设置-定位模块.png =300x)|**定位模块**：<br /><br />- 未启用|

### 4、系统设置

| 系统设置 | 说明 |
| :-: | - |
| ![系统设置-系统参数](/产品列表/车载调度/车载机/TM8760/系统设置-系统参数.png =300x) | **系统参数**：<br /><br />- 设置系统语言 |
| ![系统设置-视频编码](/产品列表/车载调度/车载机/TM8760/系统设置-视频编码.png =300x) | **视频编码**：<br /><br />- 设置各个通道的码流、分辨率 |
| ![系统设置-AD配置](/产品列表/车载调度/车载机/TM8760/系统设置-AD配置.png =300x) | **AD配置**：<br /><br />①、进入方法：`拨号 --> ### --> 呼叫`。<br /><br />②、AD芯片说明：<br />- AD1：控制1、2、3、4。<br />- AD2：控制5、6、7、8。<br /><br />③、port说明：<br />- port1：控制AD芯片前两路<br />（AD1时，控制1、2，AD2时，控制5、6）。<br />- port2：控制AD芯片后两路。 |

### 5、主动安全

| 主动安全 | 说明 |
| :-: | - |
| ![主动安全](/产品列表/车载调度/车载机/TM8760/主动安全.png =300x) | **主动安全**：<br /><br />- 设置主动安全IP、域名、抓拍通道信息<br /><br />★ 提示：只有对接主动安全的程序才有此界面 |
